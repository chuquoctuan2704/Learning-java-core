...
public class Exercises {
  public static void main(String[] args) {
    String[][] arr2DimName = new String[3][3];
    ...
    while (action != 0) {
      if (action == 1) {
        ...
        if (row >= 0 && row < arr2DimName.length && column >= 0 && column < arr2DimName[row].length) {
          ...
          arr2DimName[row][column] = editName;
        } else {
  ...
  static void showList(String[][] arr2DimName) {
    System.out.println("===============================================");
    System.out.println("Danh sách tên người");

    for (int i = 0; i < arr2DimName.length; i++) {
      for (int j = 0; j < arr2DimName[i].length; j++) {
        System.out.print((i + 1) + "." + (j + 1) + ". " + arr2DimName[i][j] + "    ");
      }
      System.out.println();
    }
